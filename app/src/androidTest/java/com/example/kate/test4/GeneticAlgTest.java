...
public class GeneticAlgTest {
    ...
    //число поколений
    private int populationLength = 100;
    ...
    public GeneticAlgTest( int persForDamageLength,List spritesPlayer, List spritesBot, List showRoundPlayer) {
    ...
    protected void setXY() {
        ...
        int i = 0;
        for (SpriteTest sprite : spritesPlayer) {
            if (!sprite.getDead()) {
        ...
        for (SpriteTest sprite : spritesBot) {
            if (!sprite.getDead()) {
                xForAlg[i] = sprite.getX();
                yForAlg[i] = sprite.getY();
            }
